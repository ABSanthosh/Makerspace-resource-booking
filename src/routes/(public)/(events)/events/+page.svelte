<script lang="ts">
  ...
  $: activeEvents = data.allEvents.filter(
    (event) =>
  ...
  $: pastEvents = data.allEvents.filter(
    (event) => new Date(event.endTime) < new Date() && event.status === 'COMPLETED'
  );
</script>
  ...
  <section class="LandingPage__hero">
    ...
    <div class="LandingPage__hero--events">
      {#if activeEvents.length > 0}
        ...
        {/each}
      {:else}
        <i class="ChrispMessage" data-type="info" data-format="box">No active events</i>
      {/if}
    </div>
    <h3 class="w-100">
      Past Events
    ...
    <div class="LandingPage__hero--events">
      {#if pastEvents.length > 0}
        ...
        {/each}
      {:else}
        <i class="CrispMessage w-100" data-type="info" data-format="box">No past events</i>
      {/if}
    </div>
  </section>
</div>
