...
export function getSlots(data: ISlot): IRange {
  ...
  if (currentDay) {
    ...
    start.setHours(Number(instance.start.split(":")[0]), Number(instance.start.split(":")[1]), 0, 0);
    ...
    for (let i = start.getTime(); i < end.getTime(); i += slotSize * 60000) {
      allSlots[new Date(i).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false })] = new Date(i);
    }
  } else {
    return {};
  }
  ...
  booked.forEach((item) => {
    if (item.start.getDate() === currentDay!.getDate() && item.end.getDate() === currentDay!.getDate()) {
      for (let i = item.start.getTime(); i < item.end.getTime(); i += slotSize * 60000) {
        bookedSlots[new Date(i).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false })] = new Date(i);
      }
    }
  });
  ...
  carted.forEach((item) => {
    if (item.start.getDate() === currentDay!.getDate() && item.end.getDate() === currentDay!.getDate()) {
      for (let i = item.start.getTime(); i < item.end.getTime(); i += slotSize * 60000) {
        cartedSlots[new Date(i).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false })] = new Date(i);
      }
    }
  });
